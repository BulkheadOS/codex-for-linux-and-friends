...
export function patchLinuxOwlFeatureBindingSource(source) {
  try {
    ...
    if (didPatch || source.includes(owlFeatureFallbackMarker)) {
...
function findOwlFeatureBindingPatch(source) {
  ...
  if (source.includes("electron_common_owl_features") && source.includes("_linkedBinding")) {
...
function patchLinuxWindowTransparency(source) {
  ...
  const replacement = anchor.replace(
    `frame:!1,transparent:${transparentVar},hasShadow:${shadowVar},`,
    `frame:${platformVar}===\`linux\`,transparent:${platformVar}===\`linux\`?!1:${transparentVar},hasShadow:${shadowVar},`
  );
